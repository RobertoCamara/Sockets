...
namespace FTPClient
{
    class Program
    {
        ...
        {
            string host = @"ftp://999.999.999.99/";
            string user = "user_name";
            string password = "pws";

            //upload
            FtpClient ftp = new FtpClient(host, user, password);
            ftp.upload("test1.txt", @"C:\Source\test1.txt");

            //download
            string[] simpleDirectoryListing = ftp.directoryListSimple("/");
            for (int i = 0; i < simpleDirectoryListing.Count(); i++)
            {
                if (simpleDirectoryListing[i] != "")
                {
                    Console.WriteLine(simpleDirectoryListing[i]);
                    ftp.download(simpleDirectoryListing[i], @"C:\Source\" + simpleDirectoryListing[i]);
                }
            }
